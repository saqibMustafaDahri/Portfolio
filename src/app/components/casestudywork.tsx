...
function WorkCard({ item }: { item: WorkItem }) {
    ...
    if (item.layout === "full") {
        return (
            <Link href={item.href} className={`block relative w-full h-[300px] md:h-[560px] overflow-hidden py-4 bg-white ${item.hasBorder ? "border border-[#D4D4D4]" : ""}`}>
                <div className="relative w-full h-full group cursor-pointer overflow-hidden">
                    <Image src={item.src} alt={item.alt} fill priority className="object-cover" />
                    <div
                        className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6"
                        style={{ background: "linear-gradient(180deg, rgba(255, 255, 255, 0) -23.89%, rgba(0, 0, 0, 0.8) 100%)" }}
                    >
                        <h3 className="text-white text-2xl md:text-[32px] font-semibold leading-tight">
    ...
    return (
        <Link href={item.href} className={`relative w-full md:w-[584px] h-[400px] md:h-[560px] group cursor-pointer overflow-hidden block ${item.hasBorder ? "border border-[#D4D4D4]" : ""}`}>
            {item.useFill ? (
                <Image src={item.src} alt={item.alt} fill className="object-cover" />
            ) : (
                <Image src={item.src} alt={item.alt} width={584} height={560} className="w-full h-full object-cover" />
            )}
            <div
                className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6"
                style={{ background: "linear-gradient(180deg, rgba(255, 255, 255, 0) -23.89%, rgba(0, 0, 0, 0.8) 100%)" }}
    ...
    return (
        <div className="max-w-7xl mx-auto grid gap-[24px] md:gap-[48px] mt-[24px] md:mt-[48px] px-0 xl:px-0">
            ...
            <div className="w-full h-auto md:h-[190px] flex items-center md:items-end justify-center py-[40px] md:py-0">
                <Link
                    href="/industrialbrand"
                    className="w-full h-auto md:h-[190px] flex items-center md:items-end justify-center py-[40px] md:py-0"
                >
                    <div className="bg-primary rounded-full w-[100px] h-[100px] md:w-[120px] md:h-[120px] flex flex-col justify-center items-center cursor-pointer hover:scale-105 transition-transform">
                        <ArrowUpRight />
                        <p className="text-white text-[14px] md:text-[16px] leading-[15px] w-[56px] text-center pt-1">See All Work</p>
                    </div>
                </Link>
            </div>
        </div>
